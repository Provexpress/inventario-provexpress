...
async function loadData() {
  try {
    ...
    // Sort products by official email section order
    const sectionOrder = ['EQUIPOS PARA RENTA', 'DELL', 'HP', 'LENOVO', 'MONITORES', 'IMPRESORAS', 'CELULARES', 'ACCESORIOS Y PERIFÉRICOS'];
    ALL.sort((a, b) => {
      let idxA = sectionOrder.indexOf(a.seccion);
      let idxB = sectionOrder.indexOf(b.seccion);
      if (idxA === -1) idxA = 99;
      if (idxB === -1) idxB = 99;
      return idxA - idxB;
    });
...
function fmtDate(s) {
  if (!s) return '—';
  const d = new Date(s);
  return isNaN(d) ? String(s).slice(0, 10) : d.toLocaleDateString('es-CO', { day: '2-digit', month: '2-digit', year: 'numeric' });
}
...
function renderDashboard() {
  ...
  setText('kpiValue', fmtCOP(totalVal));
...
function applyFilters() {
  ...
  SHOWN = ALL.filter(p => {
    const s = (p.producto + ' ' + p.np + ' ' + p.marca + ' ' + p.seccion).toLowerCase();
      ...
      html += `
        <table border="0" cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;margin-bottom:8px;border:1px solid #E2E8F0;border-radius:7px;overflow:hidden;">
          ...
          <tr>
            ...
            <td style="padding:5px 9px;border-bottom:1px solid #E2E8F0;"><strong style="color:${qColor};font-size:12px;">${qty} UNIDADES DISPONIBLES</strong></td>
          </tr>
          <tr>
